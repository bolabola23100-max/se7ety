...
class DoctorProfileCompleteView extends StatefulWidget {
  ...
  const DoctorProfileCompleteView({super.key, this.doctorModel});
...
class _DoctorProfileCompleteViewState extends State<DoctorProfileCompleteView> {
  ...
  void initState() {
    ...
    if (widget.doctorModel != null) {
      ...
      cubit.nameController.text = widget.doctorModel!.name ?? '';
      cubit.bioController.text = widget.doctorModel!.bio ?? '';
      cubit.addressController.text = widget.doctorModel!.address ?? '';
      cubit.phone1Controller.text = widget.doctorModel!.phone1 ?? '';
      cubit.phone2Controller.text = widget.doctorModel!.phone2 ?? '';
      cubit.openHourController.text = widget.doctorModel!.openHour ?? '';
      cubit.closeHourController.text = widget.doctorModel!.closeHour ?? '';
      cubit.specialization = widget.doctorModel!.specialization;
    }
  }
    ...
    return BlocListener<AuthCubit, AuthState>(
      listener: (context, state) {
        if (state is AuthLoadingState) {
          ...
          pop(context);
          pushTo(context, Routes.doctorMainAppScreen);
